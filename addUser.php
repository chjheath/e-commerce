...
<?php
    ...
    if (isset($_GET['error'])) {
        if ($_GET['error'] == 'exists') {
            echo '<p style="color: red; position: absolute; left: 730px; top: 285px;">Failed to add user. User already exists.</p>';
        } else if ($_GET['error'] == 'invalid') {
            echo '<p style="color: red; position: absolute; left: 730px; top: 285px;">Failed to add user. One or more entries invalidated your request.</p>';
        }
    }
    ...
    if (isset($_POST['enter'])) {
        ...
        if ($enteredFirst && $enteredLast && $emailMatches && $passMatches && $enteredPhone && $enteredAddrOne && $enteredCity && $enteredState && $enteredZip) {
            ...
            if ($doesntExist) {
                if (!isset($_POST['addrTwo']) || $_POST['addrTwo'] == "") { // No Address Two supplied in form
                    ...
                    if ($stmt->execute(array($addrOne, $zip, $city, $state, "YES")) == TRUE) {
                        ...
                        if ($stmt->execute(array($first, $last, $email, $pass, $phone, $time, $addrID, $activeStatus)) == TRUE) {
                            Header ("Location:admin.php?source=reg");
                        } else {
                            //Failed adding new user
                            Header ("Location:addUser.php?error=exists");
                    ...
                    if ($stmt->execute(array($addrOne, $addrTwo, $zip, $city, $state, "YES")) == TRUE) {
                        ...
                        if ($stmt->execute(array($first, $last, $email, $pass, $phone, $time, $addrID, $activeStatus)) == TRUE) {
                            ...
                            //Failed adding new user
                            Header ("Location:addUser.php?error=invalid");
